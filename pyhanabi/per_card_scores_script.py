...
def per_card_scores_calc(batch, args, model):
    ...
    trgs = torch.zeros((80, batch_size, 7))

    # gamesteps = np.zeros(len(batch.seq_len))
    # for j in range(len(batch.seq_len)):
    #     if batch.seq_len[j] % 2 == 1:
    #         gamesteps[j] = random.randint(0, (batch.seq_len[j]-2)//2)*2 + 1
    #     else:
    #         gamesteps[j] = random.randint(0, (batch.seq_len[j]-1)//2)*2 + 1
    ...
    trgs[:,:,0] = 26
    trgs[:,:,6] = 27
    trgs[:,:,1:6] = (target == 1).nonzero(as_tuple=True)[3].reshape(80, -1, 5)
    ...
    # ck = ck.reshape(ck.size(0), ck.size(1), 5, 5)
    ...
    _, trg_mask = create_masks(obs, trgs[:, 0, :-1])

    per_card_scores = np.zeros((80,5))
    trgs = trgs.type(torch.LongTensor).to(device)
    trg_mask = trg_mask.to(device)

    for j in range(batch_size):                        #5606, 205
        interleaved[j, (int(batch.seq_len[j]))*15:] = 205
    ...
    for k in range(80):
        ...
        for j in range(len(interleaved)):
            if batch.seq_len[j] >= k+1:
                src = torch.cat([src, interleaved[j]],0)
        src = src.reshape(-1,1200)
        if src.size(0) > 0:
            src[:,15*(k+1):] = 205#5606,205
            preds = model(src, None, trgs[k, [j for j in range(len(interleaved)) if batch.seq_len[j] >= k+1], :-1], None, trg_mask)
            for j in range(5):
                loss = F.cross_entropy(preds[:,j,:].view(-1, preds.size(-1)), trgs[k, [j for j in range(len(interleaved)) if batch.seq_len[j] >= k+1],j+1].contiguous().view(-1), ignore_index = 205)
                per_card_scores[k,j] += loss.item()
    # for k in range(80):
    #     num_in_batch = 0
    #     for j in range(batch_size):
    #         if batch.seq_len[j] >= k+1:
    #             num_in_batch += 1
    #             preds = torch.tensor(rainman(np.array(obs[0:k+1, j, 378:433].to("cpu")), np.array(obs[0:k+1, j, 125:250].to("cpu")))).to("cpu")
    #             preds[preds <= 0] = -1e9
    #             for l in range(5):
    #                 if trgs[k,j,l+1].item() == 25:
    #                     loss = torch.zeros(1)
    #                 else:
    #                     loss = F.cross_entropy(preds[:,l].view(1,25), trgs[k,j,l+1].view(1).to("cpu"))
                    
    #                 per_card_scores[k,l] += loss.item()
    #     if num_in_batch > 0:
    #         per_card_scores[k,:] /= num_in_batch

            # print(src[0,k*15:(k+1)*15])
            # print(trgs[k, 0, 1:6])
            # # print(F.softmax(preds[0,0,:], dim=0))
            # # print(F.softmax(preds[0,1,:], dim=0))
            # # print(F.softmax(preds[0,2,:], dim=0))
            # # print(F.softmax(preds[0,3,:], dim=0))
            # print(F.softmax(preds[0,4,:], dim=0))
...
def parse_args():
    ...
    parser.add_argument("--burn_in_frames", type=int, default=200)
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("saves_while_training/model$_multi12_allbut6_bothplayers.pth"))
    model = model.to(device)

    model.eval()

    per_card_scores = np.zeros((80,5))
    per_card_scores_each = np.zeros((500, 80, 5))

    for i in range(500):
        print(str(i+1)+"/500...")
        batch, weight = replay_buffer.sample(2, args.train_device)
        per_card_scores_each[i,:,:] = per_card_scores_calc(batch, args, model)
        per_card_scores += per_card_scores_each[i,:,:]
        priority = torch.zeros(batch.obs["priv_s"].shape[:2], device=batch.obs["priv_s"].device) + 1.0
        priority = rela.aggregate_priority(
            priority.cpu(), batch.seq_len.cpu(), args.eta
        )
    ...
    per_card_scores_avg = per_card_scores / 500

    # per_card_scores_cov = np.zeros((80,5,5))
    # for i in range(500):
    #     for j in range(5):
    #         for k in range(j+1):
    #             per_card_scores_cov[:,j,k] += ((per_card_scores_each[i,:,j] - per_card_scores_avg[:,j])*(per_card_scores_each[i,:,k] - per_card_scores_avg[:,k]))

    np.save(os.path.join('saves_while_training', 'per_card_scores_multi12_test6.npy'), per_card_scores_avg)
    print(per_card_scores_avg)
    print("Done.")
    exit()
