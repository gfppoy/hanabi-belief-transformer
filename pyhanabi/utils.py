...
def load_agent(weight_file, overwrite, op_agent_id=None):
    """
    ...
    if op_agent_id is not None:
        if 0 <= op_agent_id < 3:
            config["num_fc_layer"] = 1
            config["skip_connect"] = False
        elif 3 <= op_agent_id < 6:
            config["num_fc_layer"] = 1
            config["skip_connect"] = True
        elif 6 <= op_agent_id < 9:
            config["num_fc_layer"] = 2
            config["skip_connect"] = False
        else:
            config["num_fc_layer"] = 2
            config["skip_connect"] = True
