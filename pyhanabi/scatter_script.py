...
def get_samples(batch, args):
    ...
    for j in range(batch_size):
        interleaved[j, (gamesteps[j]+1)*15:] = 5606 # 5606, 205
    ...
    return interleaved, trgs, gamesteps
...
def belief_run_loop(model,obs,ck,target,batch,total_losses,args,optim,stopwatch,device,train_or_eval):
    ...
    return total_losses, losses, gamesteps
...
if __name__ == "__main__":
    ...
    args = parse_args()
    args.load_model = "models/sad/sad_2p_13.pthw"
    ...
    model = Transformer(5607, 6, 256, args.N, args.num_heads)
    model.load_state_dict(torch.load("saves_while_training/model$_2player_belief_multi6_semantic_evens.pth"))
    ...
    for j in range(100):
        ...
        batch, weight = replay_buffer.sample(256, args.train_device)

        obs, target, gamesteps = get_samples(batch, args)
        obs = obs.type(torch.LongTensor)
        target = target[:, :-1].type(torch.LongTensor)
        ...
        if not grounded:
            ...
            # ck = ck.to(device)
            trg_mask = trg_mask.to(device)
            preds = model(obs, None, target[:, :-1], None, trg_mask).detach()
            for j in range(5):
                loss = F.cross_entropy(preds[:,j,:].view(-1, preds.size(-1)), target[:,j+1].contiguous().view(-1), ignore_index = 5606)
                losses.append(loss)  
            x_ent += 0.2*sum(losses)

        else:
            count = args.batchsize
            losses = np.zeros(5)
            temp_losses = np.zeros(5)
            for j in range(args.batchsize):
                notgood = False
                preds = torch.tensor(rainman(np.array(batch.obs["priv_s"][0:gamesteps[j]+1, j, 0, 378:433].to("cpu")), np.array(batch.obs["priv_s"][0:gamesteps[j]+1, j, 0, 125:250].to("cpu")))).to("cpu")
                preds[preds <= 0] = -1e9
                for l in range(5):
                    if target[j,l+1].item() != 25:
                        loss = F.cross_entropy(preds[:,l].view(1,25), target[j,l+1].view(1).to("cpu")).item()
                        if loss > 20:
                            notgood = True
                            count -= 1
                            break
                ...
                if not notgood:
                    for l in range(5):
                        losses[l] += temp_losses[l]
                        
            x_ent += 0.2*sum(losses)/count
