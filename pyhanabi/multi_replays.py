...
if __name__ == "__main__":
    ...
    if args.num_policies_loaded < 12:
        ...
        torch.save(new_batches_src, "batches_src"+str(args.num_policies_loaded)+".pt")
        torch.save(new_batches_trg, "batches_trg"+str(args.num_policies_loaded)+".pt")

        args.num_policies_loaded += 1

        print("Finished policy rollout collection in " + str((time.time()-start)) + "s")
        run_again("python multi_replays.py", args.num_policies_loaded, os.getpid())

    else:
        print("Write policies...")

        batches_src = torch.cat([torch.load("batches_src0.pt"), torch.load("batches_src1.pt"), torch.load("batches_src2.pt"), 
                                torch.load("batches_src3.pt"), torch.load("batches_src4.pt"), torch.load("batches_src5.pt"),
                                torch.load("batches_src6.pt"), torch.load("batches_src7.pt"), torch.load("batches_src8.pt"),
                                torch.load("batches_src9.pt"), torch.load("batches_src10.pt"), torch.load("batches_src11.pt")], 0)

        batches_trg = torch.cat([torch.load("batches_trg0.pt"), torch.load("batches_trg1.pt"), torch.load("batches_trg2.pt"), 
                                torch.load("batches_trg3.pt"), torch.load("batches_trg4.pt"), torch.load("batches_trg5.pt"),
                                torch.load("batches_trg6.pt"), torch.load("batches_trg7.pt"), torch.load("batches_trg8.pt"),
                                torch.load("batches_trg9.pt"), torch.load("batches_trg10.pt"), torch.load("batches_trg11.pt")], 0)
        ...
        run_again("python multi_replays.py", 0, os.getpid())
